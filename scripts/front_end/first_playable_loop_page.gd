...
func _refresh_camp_world_host_state(player_state) -> void:
	...
	root_layout.visible = not show_world_camp
	camp_viewport_host.visible = show_world_camp
	...
	if enable_trace_logging:
		print("[CampHost.trace] phase=refresh show_world_camp=%s root_visible=%s host_visible=%s location=%s active_page=%s" % [
			str(show_world_camp),
...
func _on_camp_interaction_activated(_route_id: StringName, action_id: StringName, page_id: StringName) -> void:
